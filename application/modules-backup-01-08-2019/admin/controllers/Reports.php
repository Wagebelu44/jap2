...
class Reports extends CI_Controller {
	...
	public function excel_report()
	{
		...
		$objPHPExcel->getActiveSheet()->setCellValue('C1', 'Card Value');
		$objPHPExcel->getActiveSheet()->setCellValue('D1', 'Gift Cards Code');
		$objPHPExcel->getActiveSheet()->setCellValue('E1', 'Customer');
		$objPHPExcel->getActiveSheet()->setCellValue('F1', 'Email');
		$objPHPExcel->getActiveSheet()->setCellValue('G1', 'Phone No');
		$objPHPExcel->getActiveSheet()->setCellValue('H1', 'Transaction Id');
		$objPHPExcel->getActiveSheet()->setCellValue('I1', 'Transaction Amount');
		...
		foreach ($gift_cards_report as $report) {
			...
			$objPHPExcel->getActiveSheet()->setCellValue('B'.$new_row, $report['gift_card_title']);
			$objPHPExcel->getActiveSheet()->getStyle('B'.$new_row)->applyFromArray($style_header);

			$objPHPExcel->getActiveSheet()->setCellValue('C'.$new_row, CURRENCY.$report['gift_card_value']);
			$objPHPExcel->getActiveSheet()->getStyle('C'.$new_row)->applyFromArray($style_header);

			$objPHPExcel->getActiveSheet()->setCellValue('D'.$new_row, $report['gift_card_code']);
			$objPHPExcel->getActiveSheet()->getStyle('D'.$new_row)->applyFromArray($style_header);

			$objPHPExcel->getActiveSheet()->setCellValue('E'.$new_row, $report['first_name'].' '.$report['last_name']);
			$objPHPExcel->getActiveSheet()->getStyle('E'.$new_row)->applyFromArray($style_header);

			$objPHPExcel->getActiveSheet()->setCellValue('F'.$new_row, $report['email']);
			$objPHPExcel->getActiveSheet()->getStyle('F'.$new_row)->applyFromArray($style_header);

			$objPHPExcel->getActiveSheet()->setCellValue('G'.$new_row, $report['phone_no']);
			$objPHPExcel->getActiveSheet()->getStyle('G'.$new_row)->applyFromArray($style_header);

			$objPHPExcel->getActiveSheet()->setCellValue('H'.$new_row, $report['trx_id']);
			$objPHPExcel->getActiveSheet()->getStyle('H'.$new_row)->applyFromArray($style_header);

			$objPHPExcel->getActiveSheet()->setCellValue('I'.$new_row, CURRENCY.$report['trx_amount']);
