...
class Home_model extends CI_Model {
	...
	public function temporary_payment($data)
	{
		...
		if($insert_id > 0) {
			...
			if($card_code_res->num_rows() > 0) {
				...
				$this->db->set('hold_date',date('Y-m-d H:i:s'));
				...
				$this->db->set('payment_id',NULL);
				$this->db->where('trx_id', NULL);
				$this->db->where('hold_date <' , date('Y-m-d H:i:s', strtotime(date('Y-m-d H:i:s')."-10 minutes")));
				$query = $this->db->update('gift_card_codes');
				...
				$this->db->order_by('id', 'ASC'); 
				$this->db->limit(1); 
				$card_code_res1 = $this->db->get();
				$card_code = $card_code_res1->row_array();

				if($card_code_res1->num_rows() > 0) {
	...
	public function insert_sell_out_card($resp, $trx_id, $trx_amount, $voucher_code, $card_id, $first_name, $last_name, $email, $country, $username)
	{
	...
	public function get_voucher_image($card_id)
	{
		$this->db->select('*');
		$this->db->from('gift_cards');
